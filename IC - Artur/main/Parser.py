...
key = "COLOCAR CHAVE API"  # Substitua pela sua chave API
model = None

def conf_modelo():
    global model
    configure(api_key=key)
    model = GenerativeModel('COLOCAR MODELO DA LLM')


def ler_arquivo(caminho_arquivo):
    # vai ler o arquivo dependendo se é .tex ou .pdf
...
def extract_questions(text):
    ...
    for choices in re.finditer(r'\\begin{choices}.*?\\end{choices}', text, re.DOTALL):
        # choices_block = pega so as alternativas; statement = pega o enunciado; extracted adiciona os dois no vetor
...
def restructure_tex(text):
    # separa as questões que estão no arquivo,
...
def LLM_getAnswer(input):
    answer = model.generate_content(input)
    return answer.text
