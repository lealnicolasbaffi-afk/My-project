...
# 1. URL padrão da API local do LM Studio.
# 2. Insira o identificador do modelo baixado/carregado no LM Studio.
# =====================================================================
LM_STUDIO_URL = "http://localhost:1234/v1/chat/completions"
MODELO_LM = "qwen2.5-7b-instruct"


def conf_modelo():
    # Mantida para compatibilidade de estrutura do seu script.
    pass
...
def extract_questions(text):
    ...
    for choices in re.finditer(
        r"\\begin{choices}.*?\\end{choices}", text, re.DOTALL
    ):
...
def restructure_tex(text):
    ...
    for i, (statement, choices_block) in enumerate(questions):
        ...
        question_text, choices, gabarito_comparacao, salva_gabarito = (
            shuffle_and_format(statement, choices, correct)
        )
...
def compare_answer(correct, string_answer):
    ...
    for i, (gab, resp) in enumerate(zip(correct, answers)):
        ...
        with open("results.tex", "a", encoding="utf-8") as arquivo:
            arquivo.write(
                f"Questão {i + 1}: {right_answers}/{total} - {', '.join((set_resp))}\n"
            )
...
def parser(test, instrucao):
    conf_modelo()
    formatted_questions, gabarito_comparacao, gabarito_final = restructure_tex(
        ler_arquivo(test)
    )
    save_choice(gabarito_final)
    input_text = (
        ler_arquivo(instrucao) + "\n\n" + "\n\n".join(formatted_questions)
    )
